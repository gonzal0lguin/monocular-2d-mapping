...
{
public:
    LocalOccupancyGridConcatenator() : nh("~")
    {
        odom_sub = nh.subscribe("/odom", 10, &LocalOccupancyGridConcatenator::odomCallback, this);
        local_occ_grid_sub = nh.subscribe("/local_occ_grid", 10, &LocalOccupancyGridConcatenator::localOccGridCallback, this);
        global_occ_grid_pub = nh.advertise<nav_msgs::OccupancyGrid>("/global_occ_grid", 10);

        // Initialize global occupancy grid parameters
        resolution = 1.0 / 80.0; // (meters per pixel)
        map_size = static_cast<int>(50.0 / resolution); // map size (in pixels)
    ...
    void localOccGridCallback(const nav_msgs::OccupancyGrid::ConstPtr &local_occ_grid_msg)
    {
    ...
    void updateMap(std::vector<int8_t> &map, const std::vector<int8_t> &sensor_data, const std::vector<int> &position, double yaw)
    {
        ...
        std::vector<int8_t> rotated_sensor_data;
        rotated_sensor_data.reserve(sensor_data.size());

        for (int x_ = Ll - 1; x_ >= 0; --x_)
        {
            for (int y_ = 0; y_ < Hl; ++y_)
            {
                rotated_sensor_data.push_back(sensor_data[y_ * Hl + x_]);
            }
        }
    ...
    void publishGlobalOccGrid()
    {
        ...
        global_occ_grid_pub.publish(global_occ_grid_msg);
    }
...
private:
    ...
    ros::Subscriber local_occ_grid_sub;
    ros::Publisher global_occ_grid_pub;
...
int main(int argc, char **argv)
{
    ...
    LocalOccupancyGridConcatenator local_occ_grid_concatenator;
